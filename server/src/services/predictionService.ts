...
export async function startPrediction(predictionId: string, creatorId: string) {
  const prediction = requireOwned(predictionId, creatorId);
...
function scheduleAutoLock(predictionId: string, locksAt: Date) {
  ...
  const timer = setTimeout(() => {
    lockPrediction(predictionId, null, true).catch((err) =>
...
export async function lockPrediction(predictionId: string, creatorId: string | null, automatic = false) {
  const prediction = automatic ? requireExists(predictionId) : requireOwned(predictionId, creatorId as string);
...
export async function resolvePrediction(predictionId: string, creatorId: string, winningOutcomeId: string) {
  const prediction = requireOwned(predictionId, creatorId);
...
export async function updatePrediction(predictionId: string, creatorId: string, input: UpdateInput) {
  const prediction = requireOwned(predictionId, creatorId);
...
export async function cancelPrediction(predictionId: string, creatorId: string) {
  const prediction = requireOwned(predictionId, creatorId);
...
export async function listPredictions(creatorId: string, statusFilter?: string) {
  const predictions = predictionsRepo.listByCreator(creatorId, statusFilter);
  return Promise.all(predictions.map((p) => withOutcomeTotals(p.id)));
}

export async function getActivePrediction(creatorId: string) {
  const active = predictionsRepo.findActiveByCreator(creatorId);
...
function requireOwned(predictionId: string, creatorId: string) {
  const prediction = requireExists(predictionId);
  if (prediction.creatorId !== creatorId) {
    throw new ApiException(403, "FORBIDDEN", "You do not have access to this prediction.");
  }
  return prediction;
}
